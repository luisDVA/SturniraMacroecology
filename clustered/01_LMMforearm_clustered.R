...
sturnfull <- read.csv(here("data","flwbiocl.csv"),stringsAsFactors = FALSE) %>% filter(!is.na(sex))
...
# write to disk
write.csv(FLeffectsDF,here("lmmEffects","clustered","FL.csv"),row.names = FALSE)
# anova
...
# write to disk
write.csv(tidyAovOut,here("lmmEffects","clustered","aovFL.csv"),row.names = FALSE)
...
# write to disk
write.csv(forearmLMM,here("lmmEffects","clustered","FLoutput.csv"),row.names = FALSE)
