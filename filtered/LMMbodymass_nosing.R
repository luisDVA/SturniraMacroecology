...
# write to disk
write.csv(BMeffectsDF,here("lmmEffects","filtered","Bmass.csv"),row.names = FALSE)

# anova
aovOut <- Anova(mod1A,type=2) #car package (analysis of deviance)
tidyAovOut <- aovOut %>% tidy %>% rename(Chisq="statistic") %>% mutate(measurement="body mass") %>% select(measurement,everything())
# write to disk
write.csv(tidyAovOut,here("lmmEffects","filtered","aovBM.csv"),row.names = FALSE)
...
# write to disk
write.csv(bodymassLMM,here("lmmEffects","filtered","Woutput.csv"),row.names = FALSE)
...
resPermOut <- resPermTest %>% mutate(measurement="Body mass") %>% select(measurement,everything())
# write to disk
write.csv(resPermOut,here("lmmEffects","filtered","BMDDiffs.csv"),row.names = FALSE)
