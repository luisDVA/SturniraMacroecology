...
sturnfull <- read.csv(here("data","flwbiocl.csv"),stringsAsFactors = FALSE, encoding = "UTF-8") %>% filter(!is.na(W)) %>% filter(!is.na(sex))
...
# write to disk
write.csv(BMeffectsDF,here("lmmEffects","Bmass.csv"),row.names = FALSE)
...
# write to disk
write.csv(tidyAovOut,here("lmmEffects","aovBM.csv"),row.names = FALSE)
...
# write to disk
write.csv(bodymassLMM,here("lmmEffects","Woutput.csv"),row.names = FALSE)
